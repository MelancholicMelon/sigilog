...
import { Bot, Gauge, Inbox, Radio, Scale, Stamp } from 'lucide-react'
import { useStore } from '../useStore'
import type { LedgerRecord, EnvelopeMeta } from '../types'

const AGENT_META: Record<string, { Icon: typeof Bot; label: string; color: string }> = {
  'intake-agent':     { Icon: Inbox, label: 'Loan Intake',        color: 'text-sky-400' },
  'risk-agent':       { Icon: Gauge, label: 'Risk Assessment',    color: 'text-amber-400' },
  'compliance-agent': { Icon: Scale, label: 'Compliance',         color: 'text-violet-400' },
  'decision-agent':   { Icon: Stamp, label: 'Decision Authority', color: 'text-emerald-400' },
  'relay':            { Icon: Radio, label: 'Relay',              color: 'text-zinc-400' },
}

function agentMeta(id: string) {
  return AGENT_META[id] ?? { Icon: Bot, label: id, color: 'text-zinc-400' }
}
...
function buildSentMessage(env: EnvelopeMeta): Msg {
  ...
  switch (env.message_type) {
    ...
    case 'RISK_ASSESSMENT': {
      ...
      text = `Risk analysis complete. Applicant risk profile: ${risk} (score ${score}). Assessment signed and forwarded to compliance-agent for policy review.`
      tags = [`Risk: ${risk}`, `Score: ${score}`, 'Ed25519 signed', `→ ${env.recipient_ids[0]}`]
      break
    }
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1 space-y-2 overflow-auto p-2">
        {msgs.length === 0 ? (
          <div className="space-y-2 p-1 text-xs text-zinc-600">
          ...
          msgs.map((msg) => {
            ...
            return (
              <div
                key={msg.id}
                className={`card-in rounded-md border px-2.5 py-2 ${
                  isAlert
                ...
                <div className="mb-1 flex items-center justify-between">
                  <span
                    className={`flex items-center gap-1.5 text-xs font-semibold ${
                      isAlert ? 'text-red-400' : meta.color
                    }`}
                  >
